...
class M_Cart extends M_Base {
	...
	public function addProduct() {
		...
		//если такого товара ещё нет в корзине, добавляем карточку товара
			...
			$insert_query = sprintf("INSERT INTO `cart` (`product_id`, `qty`, `user_id`) VALUES (\"%d\", \"%d\", \"%d\");", $addProductID, 1, $user_id);
	...
	public function deleteGood() {
		...
		$delete_query = "DELETE FROM `cart` WHERE `cart`.`user_id` = :user_id AND `cart`.`product_id` = :good_id;";
	...
	public function clearCart() {
		$user_id = $_SESSION['user_id'];
		$delete_query = "DELETE FROM `cart` WHERE (`cart`.`user_id` = :id);";
	...
	public function ordered() {
		...
		$user_id = $_SESSION['user_id'];
		foreach ($cart as $good) {
			$insert_query = sprintf("INSERT INTO `orders` (`product_id`, `qty`, `user_id`) VALUES (\"%d\", \"%d\", \"%d\");", $good['img_id'], $good['qty'], $user_id);
			M_DB::executeQuery($insert_query);
		}
		unset($_POST['order']);
		self::clearCart();
